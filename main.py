...
deposit_rate = st.sidebar.slider("存款年化收益率（%）", 0.00, 5.00, 1.70, step=0.01, format="%.2f")
plan_rate = st.sidebar.slider("资管计划年化收益率（%）", 0.00, 6.00, 2.30, step=0.01, format="%.2f")
...
hold_days = st.sidebar.number_input("持有期（交易日）", min_value=1, max_value=252, value=7, step=1)
...
# ==== 累计概率图 ====
st.subheader("📈 累计分布 (鼠标放置在曲线可估算小于 x 的概率)")
fig_cdf = go.Figure()
